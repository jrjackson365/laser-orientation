...
author: "Justina Rodriguez Jackson aka Mac & Cheese Expert"
...
-   This appears to be student grades data or test score data

-   Is this college student data
...
```{r}
d |> 
  select(student_id, total_points_possible, total_points_earned, Gender)
```
...
```{r}
d |> 
  filter(FinalGradeCEMS > 80)
```

What happens when you change the cut-off from 70 to something else? Add
a thought (or more):

-   reduces the number of students included in the analysis
...
```{r}
ggplot(d, aes(x = (FinalGradeCEMS), y = TimeSpent)) +
  geom_point()
```
...
```{r}
m2 <- lm(FinalGradeCEMS ~ TimeSpent + subject + Gender, data = d)
summary(m2)
```
